...
link_pkg @deepseek-ai/dsh-host-webserver packages/web/host-webserver
link_pkg @deepseek-ai/dsh-client-ui-slots packages/web/client-ui-slots
# @types/node（编译类型；checkout 自带）
# 注意：若插件 node_modules 是 checkout node_modules 的 junction（合成
# checkout 部署），重建该链接会因目标路径穿过 junction 而自引用损坏——
# 因此仅在链接缺失/失效时才重建。
if [ ! -e node_modules/@types/node/package.json ]; then
